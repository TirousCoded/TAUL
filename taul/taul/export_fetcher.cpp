...
taul::source_code taul::export_fetcher(
    const grammar& gram,
    const char* fetcher,
    const char* taul_include_path,
    bool taul_includes_use_triangle_brackets,
    const char* tab) {
    TAUL_ASSERT(fetcher);
    TAUL_ASSERT(taul_include_path);
    TAUL_ASSERT(tab);

    auto _include_txt =
        [&](const char* name) -> std::string {
        return _fmt_include_txt(name, taul_include_path, taul_includes_use_triangle_brackets);
        };
